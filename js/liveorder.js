...
function formatFare(fareEur, estimateFare) {
  const value = fareEur != null ? parseFloat(fareEur) : estimateFare != null ? parseFloat(estimateFare) : null;
...
function isAssignedTabRide(ride) {
  return (ride.status || "").toLowerCase() === "assigned" || (isScheduledStatus(ride) && rideHasDriver(ride));
}
...
function populateAssignedTable(rides) {
  ...
  rides.forEach((ride) => {
    ...
    const isNewPaxDispatchOrder =
      String(ride.source || "")
        .trim()
        .toLowerCase() === "powercabs dispatch";
...
async function unassignRide(encodedRideId) {
  ...
  const ok = await showConfirmDialog({
    title: "Unassign ride?",
    message:
      "This will remove the assigned driver and move the ride back to <strong>Unassigned</strong> for re-broadcast. Continue?",
...
function showPreorderToast(message, type) {
  ...
  toast.style.cssText = `position:fixed; bottom:24px; right:24px; z-index:20001; background:${bg}; color:#fff; padding:12px 16px; border-radius:10px; font-size:0.8125rem; font-weight:600; box-shadow:0 10px 30px rgba(0,0,0,0.2); max-width:340px; font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;`;
...
// Same wording as the sidebar's own sub-links, so the navbar title
// always reads consistently with whatever's highlighted in the sidebar.
const TAB_TITLES = {
  unassigned: "Unassigned Ride",
  assigned: "Assigned Ride",
  enroute: "Enroute Ride",
  ontrip: "On-Trip Ride",
  scheduled: "Pre-Order Ride",
  cancelled: "Cancelled Ride",
  completed: "Completed Ride",
  meetgreet: "Meet & Greet",
};

// Live Orders has many tabs on one page (unassigned/assigned/pre-order/...);
// the navbar title is otherwise stuck on the static "Live Orders" from
// navbar.php regardless of which one is active. Keep it in sync here so
// it always names the tab actually showing.
function updateNavbarTabTitle(key) {
  const pageTitleEl = document.getElementById("pageTitle");
  const label = TAB_TITLES[key];
  if (!pageTitleEl || !label) return;
  pageTitleEl.textContent = `${label}`;
}

function setupTabSwitching() {
  // Default (no #tab-X in the URL yet) is the Unassigned tab; a hash
  // present below will immediately override this via the click it
  // triggers on the matching button.
  updateNavbarTabTitle(currentTab);
...
function populateEnrouteTable(rides) {
  ...
  if (!rides || rides.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="10" class="pw-loading-cell"><div class="pw-loading-inner"><i class="bi bi-inbox" style="font-size:1.7rem; color:#E4E4E7;"></i><span class="pw-loading-text">No enroute rides to show</span></div></td></tr>';
    return;
  }
  rides.forEach((ride) => {
    const name = ride.passenger_name || "N/A";
...
function populateOnTripTable(rides) {
  ...
  if (!rides || rides.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="11" class="pw-loading-cell"><div class="pw-loading-inner"><i class="bi bi-inbox" style="font-size:1.7rem; color:#E4E4E7;"></i><span class="pw-loading-text">No on-trip rides to show</span></div></td></tr>';
    return;
  }
  rides.forEach((ride) => {
    const name = ride.passenger_name || "N/A";
...
async function loadMeetGreetRides(showLoading = false) {
  ...
  if (showLoading && tbody) {
    tbody.innerHTML =
      '<tr><td colspan="9" class="pw-loading-cell"><div class="pw-loading-inner"><span class="pw-spinner-sm"></span><span class="pw-loading-text">Loading rides…</span></div></td></tr>';
  }
  try {
    ...
    if (tbody && currentTab === "meetgreet") {
      tbody.innerHTML =
        '<tr><td colspan="9" class="text-center py-4 text-danger">Error loading rides. Please refresh.</td></tr>';
    }
  }
}
...
function populateMeetGreetTable(rides) {
  ...
  if (!rides || rides.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="9" class="pw-loading-cell"><div class="pw-loading-inner"><i class="bi bi-inbox" style="font-size:1.7rem; color:#E4E4E7;"></i><span class="pw-loading-text">No meet &amp; greet rides to show</span></div></td></tr>';
    return;
  }
