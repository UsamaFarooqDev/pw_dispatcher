...
async function fetchAndUpdateDriverMarker(driverId) {
  ...
  try {
    ...
    if (isViewMode && currentRideId) {
      ...
      const resp = await fetch(`api/get_ride_location.php?ride_id=${encodeURIComponent(currentRideId)}`, {
        cache: "no-store",
      });
      if (!resp.ok) return;
...
function updateNavbarRideTitle(ride) {
  ...
  const name =
    ride.passenger_name && ride.passenger_name !== "N/A"
      ? ride.passenger_name
  ...
  if (!isNormalViewMode) {
    ...
    if (isCorporateMode) {
      const passengerLabel =
        document.querySelector('label[for="customerNameSelect"]') ||
  ...
  if (rideId && !isCorporateMode) {
    try {
      ...
      if (result.success && result.data) {
        ...
        const rideIsCorporate = String(ride.source || "")
          .toLowerCase()
        ...
        if (customerNameSelect && ride.user_id) {
          const exists = Array.from(customerNameSelect.options).some((o) => String(o.value) === String(ride.user_id));
          if (!exists) {
            const opt = document.createElement("option");
            opt.value = ride.user_id;
            opt.textContent = ride.passenger_name && ride.passenger_name !== "N/A" ? ride.passenger_name : "Passenger";
        ...
        } else if (customerNameSelect && !rideIsCorporate) {
          const customName = ride.passenger_name && ride.passenger_name !== "N/A" ? ride.passenger_name : "Passenger";
          customerNameSelect.innerHTML = "";
          const opt = document.createElement("option");
          opt.value = "__custom__";
          opt.textContent = customName;
          customerNameSelect.appendChild(opt);
          customerNameSelect.value = "__custom__";
        ...
        if (timeSource) {
          ...
          if (timeInput && !isNaN(rideDate.getTime())) {
            timeInput.value =
              String(rideDate.getHours()).padStart(2, "0") + ":" + String(rideDate.getMinutes()).padStart(2, "0");
          }
        }
  ...
  drivers.forEach((driver) => {
    ...
    item.innerHTML = `
      <div class="d-flex align-items-center gap-2">
        <div style="width:30px; height:30px; border-radius:50%; background:linear-gradient(135deg,#f37a20,#d96010); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:10px; flex-shrink:0;">${name
          .trim()
          .split(/\s+/)
          .map((p) => p[0])
          .slice(0, 2)
...
async function assignDriver() {
  ...
  if (!currentRideId && !currentCorpId) {
    oaShowToast(
      "No order selected. Please open this page from an order (e.g. from the orders list) to assign a driver.",
    );
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      const modal = new bootstrap.Modal(document.getElementById("driverAssignedModal"));
      modal.show();
...
function applyViewModeLayout() {
  ...
  const setMapHeight = () => {
    ...
    if (mapEl) {
      ...
      mapEl.style.minHeight = availH + "px";
    }
...
function createPinIcon(color) {
  const svg = encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="28" height="36" viewBox="0 0 28 36">' +
      '<path d="M14 0C6.27 0 0 6.27 0 14c0 9.63 14 22 14 22S28 23.63 28 14C28 6.27 21.73 0 14 0z" fill="' +
      color +
      '"/>' +
...
function oaTripDestinationFor(driver) {
  ...
  let lat = null,
    lng = null;
  if (onTrip) {
    lat = num(driver.dest_lat);
    lng = num(driver.dest_lng);
    if (lat === null || lng === null) {
      lat = num(driver.pickup_lat);
      lng = num(driver.pickup_lng);
    }
  } else if (prePickup) {
    lat = num(driver.pickup_lat);
    lng = num(driver.pickup_lng);
    if (lat === null || lng === null) {
      lat = num(currentPickupLat);
      lng = num(currentPickupLng);
    }
    if (lat === null || lng === null) {
      lat = num(driver.dest_lat);
      lng = num(driver.dest_lng);
...
function populateDispatcherOverlay(ride) {
  ...
  const tripStartedTimeEl = document.getElementById("overlayTripStartedTime");
  if (tripStartedTimeEl) {
    tripStartedTimeEl.textContent = formatOverlayDateTime(ride.trip_started_at) || "Not yet";
  }
...
function updateDispatcherOverlayFromDriver(loc) {
  ...
  if (loc.trip_started_at) {
    const tripStartedTimeEl = document.getElementById("overlayTripStartedTime");
    if (tripStartedTimeEl) tripStartedTimeEl.textContent = formatOverlayDateTime(loc.trip_started_at) || "Not yet";
  }
}
...
function placeViewModeMarkers() {
  ...
  if (!viewModePickupMarker) {
    ...
    const iw = new google.maps.InfoWindow({
      content:
        '<div style="font-size:13px;font-weight:700;color:#18181B;">Pickup</div>' +
  ...
  if (!viewModeDropoffMarker) {
    ...
    const iw = new google.maps.InfoWindow({
      content:
        '<div style="font-size:13px;font-weight:700;color:#18181B;">Dropoff</div>' +
        '<div style="font-size:12px;color:#52525B;margin-top:2px;">' +
...
function oaBuildDriverIcon(bearingDeg, rideStatus) {
  ...
  const col = ["on_trip", "started", "in_progress", "trip_started"].includes(s)
    ? "#3B82F6"
    : ["arrived_at_pickup", "driver_arrived", "arrived"].includes(s)
      ? "#22C55E"
      : ["assigned", "accepted", "driver_accepted"].includes(s)
        ? "#f37a20"
        : "#f37a20";
...
function showMapLoadingSkeleton() {
  ...
  skeleton.style.cssText =
    'position:absolute;inset:0;z-index:25;background:#F4F4F5;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;';
  skeleton.innerHTML = `
...
function updateRideProgressCard(status) {
  ...
  if (isCompleted) {
    ...
    iconCls = "bi-arrow-right-circle-fill";
    bg = "#F5F3FF";
    border = "#DDD6FE";
    color = "#7C3AED";
  } else if (isCancelled) {
    msg = "Ride has been cancelled.";
    iconCls = "bi-x-circle-fill";
    bg = "#FFF1F2";
    border = "#FFE4E6";
    color = "#E11D48";
  } else if (s === "searching") {
    msg = "Searching for a nearby driver...";
    iconCls = "bi-broadcast";
    bg = "#FEF3C7";
    border = "#FDE68A";
    color = "#D97706";
  } else {
...
function showCompletionOverlay() {
  ...
  overlay.style.cssText =
    'position:absolute;inset:0;z-index:20;background:rgba(255,255,255,0.92);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;backdrop-filter:blur(4px);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;';
  overlay.innerHTML = `
